...
### Presentation
- `src/components/` is split into `sections/` (homepage sections, take a `t` prop: HeroExperience, FeatureGrid, Sectors, LogosBand, Personas, Testimonials, About, Pricing, DemoForm, Trust), `ui/` (primitives: Button, Container, Reveal, AnimatedHeadline, AnimatedStats, ParallaxGlow, Logo, ThemeToggle), `layout/` (Navbar, Footer, LocaleSwitcher), `mockups/` (hand-built SVG/CSS product visuals — `FeatureSubVisuals.tsx` is the largest file in the repo at ~860 lines), `blog/`, `policies/`.
- The hero is `HeroExperience.tsx` — a scroll-driven pinned journey (460vh track, sticky 100svh stage): Web Panel → phone (QR menu) → POS → kiosk. framer-motion (`useScroll` + `useSpring`) owns the scroll progress, intro, stage copy and rail; the devices are real 3D in `HeroDeviceScene.tsx` (three.js via `@react-three/fiber` + `@react-three/drei`): RoundedBox bodies, screens painted onto 2×-resolution `CanvasTexture`s by the `paint*` functions (pure WebGL — drei `Html` screens were tried and dropped because they failed to render in some environments), per-device blob shadows, Lightformer env (no external HDR fetch — CSP/offline safe). Devices ENTER from depth (never clip the right edge) and EXIT left over the copy column while fading; the Rig shifts the scene right on landscape / down+smaller on portrait. `WINDOWS` timing exists in BOTH files — keep them in sync. Stage copy lives in the dictionaries under `hero.journey.stages`.
...
- The design language is "iOS liquid glass": `.liquid-glass`, `.liquid-glass-strong`, `.liquid-glass-sm`, `.liquid-glass-input`, `.nav-glass` (+ `.nav-glass-clear`, the transparent at-top state the scroll-aware Navbar toggles from) and `.menu-glass` (heavier-tint glass for the desktop mega-menu dropdowns), plus `.liquid-card` — one shared hover (lift + rose glow) used by every glass content card so the sections stay identical. The full-screen mobile menu deliberately uses near-opaque `bg-bg/95` instead of glass — backdrop-filter can't be relied on behind a full-viewport panel. `.mockup-container` is deliberately always dark. Accent is rose `#f43f5e`.
- Layout width: everything uses `.container-page` (max-width 1440px) via `<Container>`. Keep new sections on it.
- `framer-motion` (scroll choreography) and `three`/`@react-three/fiber`/`@react-three/drei` (device scene) are used only by the hero pair (`HeroExperience.tsx` + `HeroDeviceScene.tsx`). Everything else animates with hand-rolled `IntersectionObserver` + inline transitions (`Reveal`) or CSS keyframes in `globals.css`. Note: R3F augments the global JSX namespace — that's why `Container`'s `as` prop is typed `keyof HTMLElementTagNameMap`, not `React.ElementType`.
